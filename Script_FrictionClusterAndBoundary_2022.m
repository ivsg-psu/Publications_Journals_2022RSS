...
% fill data for the grids where no measurement is obtained
...
set(axes_handle, 'FontName', 'Times New Roman', 'FontSize', Axis_label_FontSize+2);
zlabel('friction\_coeficient')
saveas(h_fig,'Images\friction_mean_filling.svg')
%
h_fig = figure(261);
...
set(axes_handle, 'FontName', 'Times New Roman', 'FontSize', Axis_label_FontSize+2);
zlabel('friction\_coeficient')
saveas(h_fig,'Images\friction_confInterval_filling.svg')

%%s
if flag.doDebug
    h_fig = figure(98471); % comparision
    set(h_fig,'Name','true friction and filled measurement average');
    clf
    subplot(2,1,1)
    surf(FrictionGrid_ref.position_s,FrictionGrid_ref.position_t,FrictionGrid_ref.friction)
    hold on
...
%% ======== Section 6: cluster the filling friction measurement data ===
% use K-means to find the cluster, then partition each cluster into
% bounding box, also boundarymask is used find the cluater.

%%step 6.1: cluster friction using the k-means
...
%%step 6.2: find the super pixel using the k-means cluster results
...
%%step: find the boundaries using the boundarymask functon
...
%%step 6.3: find sub cluster label using location connection
...
set(axes_handle, 'FontName', 'Times New Roman', 'FontSize', Axis_label_FontSize+2);
zlabel('friction\_coeficient')
saveas(h_fig,'Images\kmeans_cluster_boundary_confiInt.svg')
...
axes_handle = gca;
set(axes_handle, 'FontName', 'Times New Roman', 'FontSize', Axis_label_FontSize+3);
saveas(h_fig,'Images\kmeans_cluster.svg')

%% ======== Section 7: cluster the confidence interval of each sub_cluster  ===
...
if flag.cluster_confidence_interval
    ...
    set(axes_handle, 'FontName', 'Times New Roman', 'FontSize', Axis_label_FontSize+2);
    zlabel('friction\_coeficient')
    saveas(h_fig,'Images\confiInterval_cluster.svg')
    
    %% ======= Section 8: split each sub_cluster into boxes  ======
    ...
    set(axes_handle, 'FontName', 'Times New Roman', 'FontSize', Axis_label_FontSize+2);
    zlabel('friction\_coeficient')
    saveas(h_fig,'Images\friction_boxes_withConfInterval.svg')
    ...
    %%split each sub_cluster into regular rectangular
    sub_cluster_label = zeros(size(Label_matrix));
    friction_cluster = unique(Label_matrix_open);
    nbs_subcluster = length(friction_cluster);
    for i=1:nbs_subcluster
    ...
    colorbar;
    xlim([0,inf])
    saveas(h_fig,'Images\friction_boxes.svg')
...
%% ======== Section 8: critical boundaries analysis  ======
